...
function buildEmailHtml(data: {
  ...
  const { child_name, xp_this_week, sessions_this_week, streak, total_xp, avg_accuracy, top_skill, weak_skill, rapport_url } = data
...
<head>
  ...
  <title>Rapport hebdomadaire Eva Étude</title>
</head>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fff;border-radius:16px;border:1px solid #e5e7eb;overflow:hidden;">
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#4f46e5,#7c3aed);padding:32px;text-align:center;">
            ...
            <p style="color:rgba(255,255,255,0.8);margin:4px 0 0;font-size:14px;">${child_name} — Eva Étude</p>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:28px;">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td width="33%" style="text-align:center;padding:12px;background:#fff7ed;border-radius:12px;">
                  ...
                  <div style="font-size:11px;color:#6b7280;margin-top:2px;">jours consecutifs</div>
                </td>
              </tr>
            </table>
            ...
            <div style="margin-top:20px;background:#f8fafc;border-radius:12px;padding:16px;">
              <p style="margin:0 0 8px;font-size:13px;color:#374151;font-weight:600;">Precision cette semaine</p>
              ...
              <p style="margin:6px 0 0;font-size:12px;color:#6b7280;text-align:right;">${avg_accuracy}% de bonnes reponses</p>
            </div>` : ''}
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin-top:20px;">
              ...
              <tr>
                <td style="padding:10px;background:#fefce8;border-radius:10px;">
                  <span style="font-size:15px;">💪</span>
                  <span style="font-size:13px;color:#854d0e;font-weight:600;margin-left:6px;">A renforcer : ${weak_skill}</span>
                </td>
              </tr>` : ''}
            </table>
            ...
            <p style="margin:20px 0 0;font-size:13px;color:#6b7280;text-align:center;">
              Total cumule : <strong style="color:#374151;">${total_xp} XP</strong>
            </p>
        ...
        <tr>
          <td style="background:#f9fafb;border-top:1px solid #e5e7eb;padding:16px;text-align:center;">
            <p style="margin:0;font-size:11px;color:#9ca3af;">
              Eva Étude — Plateforme éducative adaptative pour le Québec<br/>
              Vous recevez cet email car vous êtes parent sur Eva Étude.
            </p>
          </td>
        </tr>
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Vérifier que le parent est connecté et possède cet enfant
    ...
    const { data: link } = await supabase
      ...
      .eq('child_id', childId)
      .single()
    if (!link) return NextResponse.json({ error: 'Enfant introuvable' }, { status: 404 })

    // Récupérer les données du rapport via admin client
    const admin = createAdminClient()
    const { data: reportData } = await admin.rpc('get_weekly_report_data', { p_child_id: childId })
    if (!reportData) return NextResponse.json({ error: 'Données indisponibles' }, { status: 500 })

    const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:3000'
    const rapport_url = `${appUrl}/dashboard/enfants/${childId}/rapport`
    ...
    const { data: emailData, error: emailError } = await resend.emails.send({
      from: 'Eva Étude <rapports@evaetude.ca>',
      to: [user.email!],
      subject: `Rapport hebdomadaire de ${reportData.child_name} — Eva Étude`,
      html,
    })
